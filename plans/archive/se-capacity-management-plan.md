...
- **Активный подпункт**: —
- **Последнее обновление**: 2026-02-24
- **Примечание**: Все фазы завершены. SE показывает сконфигурированный лимит вместо размера диска.
...
- [x] [Phase 2: Upload проверка и SystemHandler](#phase-2-upload-проверка-и-systemhandler)
- [x] [Phase 3: Helm chart и OpenAPI](#phase-3-helm-chart-и-openapi)
- [x] [Phase 4: Сборка, деплой и тестирование](#phase-4-сборка-деплой-и-тестирование)
...
- [x] **2.1 Проверка capacity в Upload**
  - **Dependencies**: None
...
- [x] **2.2 Заменить diskFn в SystemHandler**
...
- [x] **2.3 Обновить main.go и удалить disk_usage.go**
  - **Dependencies**: 2.2
...
- [x] `go build ./...` успешна
- [x] `go test ./...` проходит
- [x] Upload файла сверх лимита возвращает 507 *(верифицировано в Phase 4)*
- [x] `/api/v1/info` возвращает `total_bytes = MaxCapacity` *(верифицировано в Phase 4)*
...
- [x] **3.2 Обновить OpenAPI спецификацию**
...
- [x] **4.2 Обновить тестовые Helm values**
  - **Dependencies**: 4.1
...
- [x] Все подпункты завершены (4.1, 4.2, 4.3, 4.4)
- [x] Docker-образ собран и запушен (`v0.1.0-2`)
- [x] Тестовая среда пересоздана с `SE_MAX_CAPACITY`
- [x] `/api/v1/info` показывает корректный лимит (200MB вместо 5.4TB)
- [x] Upload сверх лимита отклоняется с 507 (`STORAGE_FULL`)
- [x] Admin UI показывает правильные данные (200.0 MB)
- [x] `go test ./...` проходит
