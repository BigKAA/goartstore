...
- [x] [Phase 2: Интернационализация (i18n)](#phase-2-интернационализация-i18n)
- [x] [Phase 3: Тестовое окружение и HTTPRoute](#phase-3-тестовое-окружение-и-httproute)
- [x] [Phase 4: Сборка, деплой и верификация](#phase-4-сборка-деплой-и-верификация)
...
- [x] **2.1 Пакет i18n и каталоги переводов**
  - **Dependencies**: None
...
- [x] **2.2 Интеграция i18n middleware в роутер**
  - **Dependencies**: 2.1
...
- [x] **2.3 Переключатель языка в UI**
  - **Dependencies**: 2.2
...
- [x] **2.4 i18n: Layouts и компоненты**
  - **Dependencies**: 2.1
...
- [x] **2.7 i18n: Handlers (сообщения об ошибках и успехе)**
  - **Dependencies**: 2.1
...
- [x] **2.8 Проверка i18n и перегенерация**
...
- [x] Библиотека i18n интегрирована и работает (кастомная реализация на stdlib + golang.org/x/text)
- [x] Каталоги `en.json` и `ru.json` содержат все 395 ключей
- [x] Переключатель языка в header работает (EN/RU toggle)
- [x] Cookie `lang` сохраняется и используется (1 год, POST /admin/set-language)
- [x] Fallback на `Accept-Language` работает
- [x] Все `.templ` файла используют `T(ctx, key)` вместо хардкода
- [x] `templ generate` и `go build` проходят без ошибок
- [x] Нет кириллицы вне каталогов переводов (кроме комментариев)
...
- [x] **3.1 HTTPRoute в тестовом Helm chart**
  - **Dependencies**: None
...
- [x] **3.2 Обновление тестовых values для i18n**
  - **Dependencies**: None
...
- [x] **3.3 Обновление Dockerfile и Makefile**
  - **Dependencies**: None
...
- [x] HTTPRoute для Admin Module добавлен в тестовый chart
- [x] `helm template` для artstore-apps генерирует корректный HTTPRoute
- [x] Тестовые values содержат все необходимые UI-конфигурации
- [x] Dockerfile корректно включает i18n каталоги (embed.FS в Go-бинарнике)
...
- [x] **4.2 Деплой тестового окружения**
  - **Dependencies**: 4.1
...
- [x] **4.4 Верификация аутентификации**
  - **Dependencies**: 4.2
...
- [x] **4.6 Верификация всех страниц UI**
  - **Dependencies**: 4.4
...
- [x] **4.10 Публикация образа в Harbor**
  - **Dependencies**: 4.9
...
- [x] **4.11 Обновление документации и архивация плана**
  - **Dependencies**: 4.10
...
- [x] Все подпункты завершены (4.1–4.11)
- [x] Docker-образ успешно собран с UI, i18n (templ + tailwind + embed)
- [x] Все страницы UI работают в тестовом кластере Kubernetes
- [x] Аутентификация через Keycloak работает end-to-end
- [x] RBAC работает: admin vs readonly
- [x] SSE обновления работают в real-time
- [x] i18n: переключение en/ru работает на всех страницах
- [x] Интеграционные тесты пройдены (36/36 PASS)
- [x] Образ опубликован в Harbor registry (v0.2.0-3)
- [x] Документация обновлена
- [ ] Оба плана перенесены в archive (после commit)
