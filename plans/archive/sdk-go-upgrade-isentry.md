...
- [x] [Phase 5: Сборка, деплой и верификация](#phase-5-сборка-деплой-и-верификация)
...
- [x] **5.3 Верификация метрик без isEntry**
  - **Dependencies**: 5.2
...
- [x] **5.4 Верификация метрик с isEntry**
  - **Dependencies**: 5.3
...
- [x] Docker образы собраны и запушены
- [x] Поды запускаются без ошибок
- [x] Метрики корректно экспортируются
- [x] isEntry=true добавляет лейбл `isentry="yes"` ко всем метрикам
- [x] isEntry=false (default) — лейбл отсутствует
