...
- **Активная фаза**: Phase 9 ✅ (завершена)
- **Активный подпункт**: —
- **Последнее обновление**: 2026-02-23
- **Примечание**: Все фазы завершены. Готово к сборке образа и деплою.
...
- [x] [Phase 5: Storage Elements](#phase-5-storage-elements)
- [x] [Phase 6: Файловый менеджер](#phase-6-файловый-менеджер)
- [x] [Phase 7: Управление доступом](#phase-7-управление-доступом)
- [x] [Phase 8: Мониторинг, SSE и настройки](#phase-8-мониторинг-sse-и-настройки)
- [x] [Phase 9: Сборка, деплой и тестирование](#phase-9-сборка-деплой-и-тестирование)
...
- [x] **5.3 Discover и регистрация SE**
  - **Dependencies**: 5.1
...
- [x] **5.4 Редактирование, удаление, синхронизация**
  - **Dependencies**: 5.1
...
- [x] Таблица SE отображает все зарегистрированные SE
- [x] Фильтрация по mode/status и поиск работают (HTMX partial swap)
- [x] Discover: ввод URL → предпросмотр данных SE
- [x] Регистрация SE: предпросмотр → кнопка → SE в таблице
- [x] Редактирование: inline-форма → сохранение → обновление страницы
- [x] Удаление: confirm → удаление (или ошибка если есть файлы)
- [x] Sync: кнопка → синхронизация → обновление данных
- [x] Детальная страница: полная информация + список файлов
- [x] RBAC: readonly видит всё, но кнопки действий скрыты
- [ ] Контейнер собирается и запускается (проверить при деплое)
...
- [x] **6.1 File list page с пагинацией**
  - **Dependencies**: None
...
- [x] **6.2 Фильтры, поиск, сортировка**
  - **Dependencies**: 6.1
...
- [x] **6.3 File detail modal и действия**
  - **Dependencies**: 6.1
...
- [x] Таблица файлов с пагинацией работает
- [x] Фильтрация по status/retention/SE/content_type работает
- [x] Поиск по имени файла работает
- [x] Сортировка по клику на заголовок работает
- [x] Modal с метаданными отображает все поля
- [x] Редактирование description/tags сохраняет изменения (admin only)
- [x] Soft delete работает с подтверждением (admin only)
- [x] «Показать удалённые» toggle работает (admin only)
- [x] RBAC: readonly — только просмотр, кнопки действий скрыты
- [ ] Контейнер собирается и запускается (проверить при деплое)
...
- [x] **7.1 Страница с табами и Users list**
  - **Dependencies**: None
...
- [x] **7.2 User detail и role overrides**
  - **Dependencies**: 7.1
  - **Description**: Реализовать:
...
- [x] **7.3 Service Accounts tab — список и создание**
  - **Dependencies**: 7.1
...
- [x] **7.4 SA: редактирование, удаление, ротация, sync**
  - **Dependencies**: 7.3
...
- [x] Табы переключаются без перезагрузки страницы (Alpine.js)
- [x] Таблица пользователей отображает данные из Keycloak
- [x] Фильтрация и поиск пользователей работают
- [x] Role override: повышение и удаление работает (admin only)
- [x] Таблица SA отображает все Service Accounts
- [x] Создание SA: форма → secret показан один раз → copy works
- [x] Редактирование SA сохраняет изменения
- [x] Удаление SA с подтверждением
- [x] Ротация secret: предупреждение → новый secret → copy
- [x] Sync SA: кнопка → результат синхронизации
- [x] RBAC: readonly — только просмотр; admin — все действия
- [ ] Контейнер собирается и запускается (проверить при деплое)
...
- [x] **8.2 Monitoring page — здоровье и задачи**
  - **Dependencies**: 8.1
...
- [x] **8.3 Prometheus-клиент (опциональный)**
  - **Dependencies**: None
...
- [x] **8.4 Графики latency (с Prometheus)**
...
- [x] **8.6 SSE-интеграция на Dashboard и SE pages**
  - **Dependencies**: 8.1
...
- [x] SSE endpoint `/admin/events/system-status` отправляет статусы каждые 15с
- [x] Monitoring page показывает здоровье всех зависимостей (real-time)
- [x] Фоновые задачи: последний запуск, результат, интервал
- [x] Алерты отображаются при проблемах (SE offline, dependency down)
- [x] Prometheus: если настроен — графики latency отображаются
- [x] Prometheus: если не настроен — placeholder с ссылкой на настройки
- [x] Settings: конфигурация Prometheus сохраняется в БД
- [x] Settings: «Проверить подключение» работает
- [x] Dashboard: статусы зависимостей обновляются через SSE
- [x] SE list: статусы SE обновляются через SSE (подготовлено через SSE endpoint, Alpine.js на Dashboard)
- [ ] Контейнер собирается и запускается (проверить при деплое)
...
- [x] **9.3 Обновление docker-compose.yaml и тестового окружения**
  - **Dependencies**: 9.1
...
- [x] **9.4 Интеграционное тестирование**
...
- [x] Все подпункты завершены (9.1–9.5)
- [ ] **Docker-образ успешно собран** с UI (templ + tailwind + embed) — проверить при деплое
- [ ] **Все страницы UI работают** в тестовом кластере Kubernetes — проверить при деплое
- [ ] **Аутентификация через Keycloak** работает end-to-end — проверить при деплое
- [ ] **RBAC** работает: admin vs readonly — проверить при деплое
- [ ] **SSE** обновления работают в real-time — проверить при деплое
- [ ] **Интеграционные тесты пройдены** — проверить при деплое
- [x] Документация обновлена
- [ ] Образ опубликован в Harbor registry — проверить при деплое
