...
- [ ] [Phase 1: Фундамент и инструментарий](#phase-1-фундамент-и-инструментарий)
...
- [ ] **1.2 Tailwind CSS Standalone CLI**
  - **Dependencies**: 1.1
...
- [ ] **1.3 Статические JS-ассеты и embed.FS**
  - **Dependencies**: 1.1
...
- [ ] **1.4 Миграция БД: таблица ui_settings**
  - **Dependencies**: None
...
- [ ] **1.5 Конфигурация UI и Makefile**
...
- [ ] `templ generate` успешно генерирует Go-код из `.templ` файлов
- [ ] `tailwindcss` компилирует CSS с кастомными дизайн-токенами
- [ ] Статические JS-файлы доступны через `embed.FS`
- [ ] `go build` проходит без ошибок с новыми зависимостями
- [ ] Миграция `003_ui_settings` применяется без ошибок
- [ ] `make build` (или `make ui-build && make build`) собирает бинарник
