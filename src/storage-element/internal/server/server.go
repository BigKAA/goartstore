	...
	if jwtAuth != nil {
		...
		router.Group(func(r chi.Router) {
			...
			r.Group(func(rr chi.Router) {
				...
				// Lock API — диагностика и очистка lock-файлов (через generated wrapper для парсинга параметров)
				rr.Get("/api/v1/locks", func(w http.ResponseWriter, r *http.Request) {
					siw := &generated.ServerInterfaceWrapper{Handler: handler, ErrorHandlerFunc: defaultErrorHandler}
					siw.ListLocks(w, r)
				})
				rr.Post("/api/v1/locks/cleanup", func(w http.ResponseWriter, r *http.Request) {
					siw := &generated.ServerInterfaceWrapper{Handler: handler, ErrorHandlerFunc: defaultErrorHandler}
					siw.CleanupLocks(w, r)
				})
			})
		})
	} else {
