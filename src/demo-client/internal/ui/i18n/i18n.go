// Пакет i18n — интернационализация Demo Client UI.
...
// Язык определяется middleware: cookie "lang" → Accept-Language → default "ru".
package i18n
...
func (b *Bundle) Translate(lang, key string) string {
	...
	// Fallback на русский (основной язык DC)
	if lang != "ru" {
		if catalog, ok := b.catalogs["ru"]; ok {
			if msg, ok := catalog[key]; ok {
				return msg
			}
		}
...
// LangFromContext извлекает язык из контекста. Default: "ru".
