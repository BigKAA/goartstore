...
// QM не создаёт таблицы (shared DB с AM), только добавляет индексы для поиска.
package database
...
// QM миграции добавляют индексы для поиска в таблицу file_registry (owned by AM).
...
func Migrate(cfg *config.Config, logger *slog.Logger) error {
	...
	dbURL := fmt.Sprintf(
		"pgx5://%s:%s@%s:%d/%s?sslmode=%s&x-migrations-table=schema_migrations_qm",
	...
	logger.Info("Миграции QM применены",
