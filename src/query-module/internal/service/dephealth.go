...
// (SEListFetcher). При старте QM список SE пуст, первые данные появляются
...
//   - tlsSkipVerify — пропускать проверку TLS-сертификатов (QM_TLS_SKIP_VERIFY)
...
func NewDephealthService(
	...
	return newDephealthService(serviceID, group, db, pgConnURL, adminModuleURL, jwksURL,
...
func newDephealthService(
	...
	opts := make([]dephealth.Option, 0, 4+len(extraOpts))
	opts = append(opts,
...
func (ds *DephealthService) Start(ctx context.Context) error {
	...
	ds.logger.Info("Мониторинг зависимостей запущен (PostgreSQL + Admin Module + Keycloak JWKS + SE sync)")
	return nil
}
